...
/* ---------- 主题输入状态共享自 useTheme（composables/useTheme.ts） ---------- */

const presets = themePresets
const { mode, committed, currentHex, palette, onActiveChange } = useTheme()
...
function onConfirm(): void {
  ...
  setTimeout(() => {
    ...
    dialogLog.value = `提交成功 · ${new Date().toLocaleTimeString()}`
    dialogVisible.value = false
  }, 800)
}

// 进入页时默认高亮 brand_4，让绑定表与色带的联动关系一目了然
selectSeed('brand')
</script>

<template>
  <main class="playground">
    <h1>OKLCH 规则引擎 · 主题变量总览</h1>
    <p class="muted">
    ...
    <section class="card">
      ...
      <div class="row">
        ...
        <HbButton type="success">成功</HbButton>
        <HbButton type="warning">警告</HbButton>
        <HbButton type="danger">危险</HbButton>
        <HbButton type="info">信息</HbButton>
        <HbButton>默认</HbButton>
      </div>
      <div class="row">
        <HbButton type="primary" @click="dialogVisible = true">
          打开 HbDialog
        </HbButton>
        <span v-if="dialogLog" class="muted">{{ dialogLog }}</span>
      </div>
    </section>
    ...
    <HbDialog
      ...
      @confirm="onConfirm"
    >
      <p>弹窗的背景、描边、文字、主按钮均由 <code>--hb-*</code> 派生结果驱动。</p>
    </HbDialog>
  </main>
</template>
